    ...
    def calc_values(self, se, glat, xx, xy, xz, zz, j_loc):
        ...
        for i_k, kv, wk in zip(glatik, glatk, glatwk):
            ...
            for i, j, m, n in itt.product(*[range(8)]*4):
                if i >= 4 and n < 4:
                    continue  # anomalous symmetry
                if j >= 4 and m < 4:
                    continue  # no anomalous dispersion
                if m >= 4 and j < 4:
                    continue
                ...
                if not((i, n) in [(1, 5), (1, 1), (5, 5), (1, 6), (2, 5), (1, 2), (6, 5), (2, 1), (5, 6), (2, 6),
                    ...
                    continue
                if xx or xy or xz:
                    dgdkx[i, n] += glatgki[i, j] * \
                        depsdkx[j, m] * glatgki[m, n]
                if xy:
                    dgdky[i, n] += glatgki[i, j] * \
                        depsdky[j, m] * glatgki[m, n]
                if xz or zz:
                    dgdkz[i, n] += glatgki[i, j] * \
                        depsdkz[j, m] * glatgki[m, n]
